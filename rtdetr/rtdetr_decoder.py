...
class RTDETRTransformer(nn.Module):
    ...
    def __init__(
        ...
        # self.interactive_class_embed = nn.Embedding(self.num_classes, hidden_dim)
        ...
        self.dec_score_head = nn.ModuleList(
            [nn.Linear(hidden_dim, self.num_classes) for _ in range(num_decoder_layers)]
        )
    ...
    def _get_decoder_input(
        ...
        # Get batch size from the memory tensor's shape
        bs, _, _ = memory.shape

        # Generate or retrieve anchors and a mask to indicate valid regions, depending on the mode
        # prepare input for decoder
        if self.training or self.eval_spatial_size is None:
            # During training or if no specific evaluation spatial size set, generate new anchors
            anchors, valid_mask = self._generate_anchors(
                spatial_shapes, device=memory.device
            )
        else:
            # During evaluation with a defined spatial size, use pre-stored anchors and masks
            anchors, valid_mask = self.anchors.to(memory.device), self.valid_mask.to(
                memory.device
            )

        # Apply the valid mask to the memory tensor, setting unmasked areas to zero
        memory = (
            valid_mask.to(memory.dtype) * memory
        )  # TODO fix type error for onnx export

        # Pass the masked memory through an encoder to get the encoded output
        output_memory = self.enc_output(memory)

        # Apply classification and bounding box heads to the encoder output
        enc_outputs_class = self.enc_score_head(output_memory)
        enc_outputs_coord_unact = self.enc_bbox_head(output_memory) + anchors

        # Select top-k queries based on class confidence
        _, topk_ind = torch.topk(
            enc_outputs_class.max(-1).values, self.num_queries, dim=1
        )

        # Gather the corresponding top-k unactivated coordinates
        ...
        # Apply sigmoid to the top-k coordinates to get bounding boxes
        enc_topk_bboxes = F.sigmoid(reference_points_unact)
        if denoising_bbox_unact is not None:
            # Concatenate denoising bounding box data if provided
            reference_points_unact = torch.concat(
                [denoising_bbox_unact, reference_points_unact], 1
            )

        # Gather the corresponding top-k logits for the class predictions
        enc_topk_logits = enc_outputs_class.gather(
            dim=1,
            index=topk_ind.unsqueeze(-1).repeat(1, 1, enc_outputs_class.shape[-1]),
        )

        # Depending on the configuration, prepare target tensor for decoder
        # extract region features
        if self.learnt_init_query:
            # If learnt initialization is enabled, use the learned target embeddings
            target = self.tgt_embed.weight.unsqueeze(0).tile([bs, 1, 1])
        else:
            # Otherwise, gather the memory outputs corresponding to the top-k indices
            target = output_memory.gather(
                dim=1,
                index=topk_ind.unsqueeze(-1).repeat(1, 1, output_memory.shape[-1]),
            )
            target = target.detach()  # Detach target to avoid gradient propagation

        if denoising_class is not None:
            # Concatenate denoising class data if provided
            target = torch.concat([denoising_class, target], 1)

        # Return all relevant tensors for the decoder input
    ...
    def forward(
        ...
        # memory.shape = [bs, seq, 256] # [16, 13125, 256]
        # spatial_shapes = [[100, 100], [50, 50], [25, 25]]
        # level_start_index = [10000, 12500, 13125]
        ...
        if cargs.late_combine and cargs.interactive:
            heatmap_scaled = torch.cat(
                attn_weights,
                1,
            )

            heatmap = heatmap_scaled.repeat(1, 1, self.hidden_dim)

            if cargs.residual:
                memory = memory + memory * heatmap
            else:
        ...
        # start = 0
        # for spatial_shape_i in range(len(spatial_shapes)):

        #     spatial_shape = spatial_shapes[spatial_shape_i]
        #     local_memory = memory[
        #         :, start : start + spatial_shape[0] * spatial_shape[1], :
        #     ]

        #     start += spatial_shape[0] * spatial_shape[1]
